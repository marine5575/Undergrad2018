...
#label.AddText("Work in Progress          Delphes Sim at #sqrt{s} = 13 TeV")
label.AddText("                        Delphes Sim at #sqrt{s} = 13 TeV")
...
###########################################################
for ch in ['ctautau_j2_b1_tau1_01']:
  out = TFile.Open(ch+'/output_'+ch+'.root')
  ...
  for method in ['BDT']:
    ...
    c1.Print('bdt_plots/overtrain_'+ch+'_'+method+'.pdf')
    l.Clear()
    label2.Clear()
    c1.Clear()
